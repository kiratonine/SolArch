...
## Дизайн-язык (S2, зафиксирован)

```text
Идея          «Запечатано на виду»: контур контейнера публичен, содержимое — нет
Бумага        холодная синеватая, не белая и не кремовая; чернила — тёмно-синие
Акцент        латунный янтарь ТОЛЬКО для денег (цена, доля автора). Больше нигде
Токены        --seal (заливки) и --seal-ink (текст, контраст AA) — не путать
