...
i18n — свой типизированный словарь (без библиотеки), en по умолчанию + ru
```
...
## Дизайн-язык (S2, палитра Almond — F134)

```text
Идея          «Запечатано на виду»: контур контейнера публичен, содержимое — нет
Бумага        тёплая миндальная, на ступень светлее Almond Hearth #EED3BA; сама #EED3BA —
              ступень поверхности (secondary/accent), не фон. Чернила — Obsidian Ink,
              тёплый почти-чёрный
Акцент        золото ТОЛЬКО для денег и кошельков (цена, доля автора, адрес). Больше нигде
Токены        --seal — яркая латунь, только поверх чернил (угол знака): по бумаге 1.8:1.
              --seal-ink — тёмная бронза, текст по бумаге (AA). Не путать
Темы          палитра — файл в src/styles/, активна одна через импорт в index.css.
              theme-paper.css — прежняя S2 (холодная бумага) для отката; при откате
              вернуть и два цвета в public/favicon.svg
Статусы       своя приглушённая шкала --state-idle/progress/ok/error
Шрифты        Unbounded (только h1 и логотип) · Golos Text (интерфейс)
              IBM Plex Mono (только машинные строки: кошельки, пути, .slr)
Тени          одна: отклик плитки каталога на hover (короткая, снизу). Остальная глубина —
              разница бумага/поверхность плюс волосяные линии
Радиусы       по роли: чипы rounded-sm, карточки rounded-lg; не один на всё
Ширина        рамка 120rem (max-w-480) — шапка, подвал, каталог и кабинет (`Container wide`);
              страницы-документы и формы — колонка 54rem по центру рамки
Корешок       левая полоса несёт статус цветом (TONE_SPINE) — только в панелях страницы
              архива в кабинете (публикация, сборка);
              у плиток (каталог, кабинет) корешка нет, статус несут бейджи
Копирайтинг   активный залог, обычный регистр; без капслок-меток, без «→» в кнопках
```

Тексты в компонентах не пишем — только через `useI18n()`. Обе локали обязаны совпадать
по ключам: `ru` типизирован как `typeof en`, пропуск падает на `tsc`.
