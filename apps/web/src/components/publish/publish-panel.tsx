 ...
 * уже проверено — чужой архив отдаётся как 404. Решает всё равно backend: кнопка
 * не «разрешает» публикацию, она её просит.
 *
 * Заранее называется одно препятствие — несобранный контейнер. USDC-аккаунт
 * препятствием до нажатия не считается: backend создаёт его в момент публикации,
 * и `payout_account_ready` до неё честно отвечает `false`. Гасить кнопку по этому
 * флагу значило бы ждать того, что случится только после нажатия, — новый автор
 * не опубликовал бы архив никогда. Если аккаунт создать не вышло, backend откажет
 * с 409, и его фраза встанет рядом с кнопкой (найдено живым прогоном на S12).
 */
type Blocker = 'notReady' | null

function blockerOf(archive: CreatorArchive): Blocker {
  return archive.technical_status === 'ready' ? null : 'notReady'
}
